# KRONOS V5 Sovereign Architecture

**File**: `KRONOS_V5_SOVEREIGN_ARCHITECTURE.md`  
...
Defines the complete system architecture of the KRONOS Hybrid Sovereign Engine — component boundaries, data flows, engine responsibilities, and the sovereign/hybrid split. All dimensions, thresholds, and configuration keys resolve exclusively via `cfg["section"]["key"]`.
...
```
═══════════════════════════════════════════════════════════════
KRONOS SYSTEM ARCHITECTURE — CAUSAL DATA FLOW
═══════════════════════════════════════════════════════════════

LAYER — RAW DATA INGESTION
  data_engine.fetch(cfg["data"]["source"], cfg["data"]["symbols"],
                    cfg["data"]["intervals"])
  → raw_candles_df   : OHLCV at cfg["feature_builder"]["interval"]
...
LAYER — CAUSAL FEATURE EXTRACTION (no future data)
  ...
  for each bar t:
    causal_slice = raw_candles_df.iloc[: t + one_i]
    trades_slice = agg_trades_df.iloc[: t + one_i]
...
LAYER — POST-DETECTION EVALUATION (only when signature_flag)
  fwd_slice = raw_candles_df.iloc[t + one_i : t + one_i + cfg["miner"]["forward_bars"]]
...
| :--- | :--- | :--- | :--- |
| Data ingestion | `data_engine.py` | Fetch, normalise, and cache raw OHLCV and AggTrades | `cfg["data"]` |
...
| Walk-forward miner | `miner_engine.py` | Shard generation, detection loop, forward metric evaluation | `cfg["miner"]` |
...
| System orchestrator | `orchestrator_engine.py` | Top-level pipeline coordination | All cfg roots |
...
| Neural orthogonal gate | **Hybrid** — frozen transformer dependency | Kronos-mini model + GPU | `cfg["miner"]["enable_kronos"]` |
...
| :--- | :--- | :--- | :--- |
| `orchestrator_engine` | `data_engine` | `data_engine.fetch(cfg)` | `cfg["data"]` |
| `orchestrator_engine` | `miner_engine` | `miner_engine.run_miner(cfg, start, end)` | `cfg["miner"]` |
...
class KRONOSOrchestrator:
    ...
    Full implementation → orchestrator_engine.KRONOSOrchestrator
    """

    def __init__(self, config: Dict) -> None:
        from orchestrator_engine import KRONOSOrchestrator as _impl
    ...
    def run(self, start_date: str, end_date: str) -> None:
        ...
        Full implementation → orchestrator_engine.KRONOSOrchestrator.run(...)
        """
...
> - Phase current: Hybrid — structural sovereign core + Kronos-mini neural gate.
> - Phase next: Replace neural slots with deterministic structural surrogates. Toggle via `cfg["miner"]["enable_kronos"]`.
> - Phase final: All slots deterministic, GPU-free, hardware-portable. Weight map covers full slot set.
