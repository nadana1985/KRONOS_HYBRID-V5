# Altcoin Signature Query Examples

**File**: `ALTCOIN_SIGNATURE_QUERY_EXAMPLES.md`  
...
Defines authoritative parameterised DuckDB query patterns for interrogating the Altcoin KRONOS signature archive. The system evaluates `cfg["universe"]["size"]` perpetual altcoins across `cfg["feature_builder"]["interval"]` intervals, processing full historical data from `cfg["data_loader"]["history_start"]` up to the present. All compute executes on `cfg["compute"]["infrastructure"]`. All filter values, column names, limit clauses, and threshold parameters are bound as parameterised variables from config. No literal values appear in query strings.
...
```
═══════════════════════════════════════════════════════════════
ALTCOIN QUERY FLOW (Scope: cfg["universe"]["size"] assets)
═══════════════════════════════════════════════════════════════

CONFIG LOAD
  cfg = load_sovereign_config()

CONNECTION INITIALISE (Infrastructure: cfg["compute"]["infrastructure"])
...
PARAMETERISED QUERY EXECUTION
  ...
  → aggregation across cross-section of assets
...
| Asset identifier column | `cfg["feature_builder"]["metadata"]["keys"]["asset_id"]` | `cfg["feature_builder"]["metadata"]["keys"]["asset_id"]` |

---

## Query Stubs

All SQL strings use `?` bindings. Column names resolved dynamically from config. Timeframes scale dynamically to `cfg["feature_builder"]["interval"]`.
...
def query_elite_signatures(con, config: Dict) -> pd.DataFrame:
    """
    Retrieves high-quality, high-conviction signatures across cfg["universe"]["size"] assets.
...
def query_by_regime(con, config: Dict) -> pd.DataFrame:
    """
    Filters signatures by target HMM regime across the cross-section.
...
def query_cross_sectional_edge_stats(con, config: Dict) -> pd.DataFrame:
    """
    Computes cross-sectional per-asset edge statistics with HAVING filter.
    Asset column, recovery column, and minimum recovery from config.

    Config refs:
      cfg["feature_builder"]["metadata"]["keys"]["asset_id"]            ← asset column
    ...
    Full implementation → database_engine.query_cross_sectional_edge_stats(con, config)
    """
    from database_engine import query_cross_sectional_edge_stats as _impl
    return _impl(con, config)
...
def query_audit_trail(con, config: Dict, start_date: str, end_date: str) -> pd.DataFrame:
    """
    Retrieves audit trail columns for a date range bounded by cfg["data_loader"]["history_start"].
