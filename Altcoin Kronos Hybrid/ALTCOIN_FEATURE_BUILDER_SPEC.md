# Altcoin Feature Builder Specification

**File**: `ALTCOIN_FEATURE_BUILDER_SPEC.md`  
...
**Depends On**: `structural_engine.py`, `neural_integration_engine.py`, `feature_builder_engine.py`, `cross_sectional_engine.py`

---

## Purpose

Defines the authoritative, mathematically pure pipeline for converting raw OHLCV candlesticks and aggregated microstructure trades into the 32-slot reversal signature DNA vector across a cross-sectional universe of altcoin perpetuals (`cfg["universe"]["size"]`). Processing is conducted at the `cfg["feature_builder"]["interval"]` timeframe, ingesting full historical data spanning from `cfg["universe"]["history_start"]` to present, distributed via `cfg["compute"]["infrastructure"]`. Every index boundary, sliding window, threshold, norm order, and column identifier resolves exclusively via config lookups. No inline numeric constants, literal array bounds, or hardcoded strings exist in this specification or its matching engine code.
...
```
══════════════════════════════════════════════════════════════════════════════════
ALTCOIN KRONOS CAUSAL FEATURE PIPELINE — bar t across universe
══════════════════════════════════════════════════════════════════════════════════
...
STEP TWO — CROSS-SECTIONAL UNIVERSE ITERATION
  # Iterate over cfg["universe"]["size"] altcoin perpetuals
  for coin in universe:
      # Process full history from cfg["universe"]["history_start"] at cfg["feature_builder"]["interval"]

STEP THREE — CAUSAL BARS SLICING (Hard Future-Data Barrier)
  # Limit input candles and aggregated trades up to index t causally for the specific coin
  one_idx        = cfg["reproducibility"]["constants"]["one_int"]
  causal_candles = coin_raw_candles.iloc[: current_idx + one_idx]
  causal_trades  = coin_agg_trades_df.iloc[: current_idx + one_idx]

STEP FOUR — STRUCTURAL SLOTS GENERATION
...
STEP FIVE — COMPOSITE VETO EVALUATION
...
STEP SIX — NEURAL ORTHOGONAL INFERENCE
  ...
  if veto_passed and cfg["miner"]["enable_kronos"]:
      # Inference dispatched to cfg["compute"]["infrastructure"]
...
STEP SEVEN — CONVICTION HISTORY (TIME WINDOW, NOT COUNT WINDOW)
...
STEP EIGHT — AUXILIARY + METADATA + ASSEMBLY
...
STEP NINE — REVERSAL SIGNATURE FLAG
...
> - Neural conviction checks across the `cfg["universe"]["size"]` scale require significant compute. For this altcoin implementation, execution is dispatched to `cfg["compute"]["infrastructure"]` (Lightning AI Cloud Mining pools).
