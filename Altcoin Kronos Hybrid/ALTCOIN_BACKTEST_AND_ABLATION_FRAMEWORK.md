# Altcoin Backtest and Ablation Framework

**File**: `ALTCOIN_BACKTEST_AND_ABLATION_FRAMEWORK.md`  
...
Defines the walk-forward backtesting and ablation study framework for Altcoin Kronos Hybrid. The scope spans `cfg["universe"]["size"]` perpetual altcoins using cross-sectional processing at a `cfg["feature_builder"]["interval"]` timeframe, utilizing `cfg["data"]["history_range"]` historical data. Execution targets `cfg["compute"]["infrastructure"]`. All fold sizes, evaluation horizons, ablation toggles, and performance targets resolve exclusively via `cfg["section"]["key"]`. The framework supports both full hybrid runs and sovereign-only structural ablations.
...
```
═══════════════════════════════════════════════════════════════
ALTCOIN BACKTEST + ABLATION PIPELINE
═══════════════════════════════════════════════════════════════
...
SIGNATURE CORPUS LOAD
  ...
  Applies to cfg["universe"]["size"] assets cross-sectionally.
...
| :--- | :--- | :--- |
| Universe size | Number of perpetual altcoins (cross-sectional) | `cfg["universe"]["size"]` |
| Feature timeframe | Trading and feature building interval | `cfg["feature_builder"]["interval"]` |
| Historical data scope | History size | `cfg["data"]["history_range"]` |
| Compute infrastructure | Target environment for massive backtesting | `cfg["compute"]["infrastructure"]` |
