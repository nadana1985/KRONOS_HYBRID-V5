# Altcoin Sovereign Validator Specification

**File**: `ALTCOIN_SOVEREIGN_VALIDATOR.md`  
...
Defines the Altcoin Kronos Hybrid hardcode validator: a CI-grade scanner that enforces the zero-inline-literal doctrine across all specification and engine files. All detection patterns, allowlists, severity rules, and exit codes resolve exclusively via `cfg["validator"]`. No regex literals appear in this document.

The validator strictly prohibits hardcoding domain values for the Altcoin system, including universe sizes, interval granularities, historical bounding dates, and cloud infrastructure identifiers.
...
## Altcoin Domain Enforcement

The hardcode validator explicitly guards against inline literals related to the Altcoin Kronos Hybrid domain constraints:

*   **Scope:** Universe size must be referenced via `cfg["universe"]["size"]` (no bare integers for the perpetual altcoin count).
*   **Timeframe:** Interval length must be referenced via `cfg["feature_builder"]["interval"]` (no bare strings for the interval duration).
*   **History:** Historical boundaries must be referenced via `cfg["data_loader"]["history_start"]` and `cfg["data_loader"]["history_end"]` (no bare genesis timestamps).
*   **Compute:** Infrastructure configuration must be referenced via `cfg["infrastructure"]["compute_provider"]` (no literal strings for cloud compute selection).
...
| :--- | :--- | :--- |
| Bare integer literal | Standalone number in code block (e.g. universe sizes) | CRITICAL |
| Bare float literal | Decimal number in code block | CRITICAL |
| Bare string literal | Hardcoded interval, platform, or date | CRITICAL |
