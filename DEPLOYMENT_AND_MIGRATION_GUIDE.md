...
**File**: `DEPLOYMENT_AND_MIGRATION_GUIDE.md`  
...
Defines the deployment contract and sovereignty migration roadmap for KRONOS. Covers environment setup, config verification, sovereignty migration phases, and rollback procedures. All environment values, migration toggles, and phase conditions resolve exclusively via `cfg["section"]["key"]`.
...
STEP — DATA LAYER INITIALISATION
  ...
  → fetches and caches raw OHLCV + AggTrades
...
| :--- | :--- | :--- | :--- |
| Current Hybrid | Structural sovereign core + Kronos-mini neural gate | `cfg["miner"]["enable_kronos"]: True` | Partial |
...
> [!IMPORTANT]
> Update `params_yaml.txt` to add the `deployment` section before running any production mining loop. All values above must resolve at runtime.
