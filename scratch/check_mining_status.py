...
print("+------------------------------------------------------------------+")
print("|       KRONOS HYBRID - Mining Intelligence Report                 |")
...
# =============================================================================
print("====================================================================")
print("  [2]  SIGNATURES FOUND SO FAR")
print("====================================================================")

df = None
try:
    if os.path.exists(compact_file):
        df = pd.read_parquet(compact_file)
        print(f"  Source         : Compact file  ({compact_file})")
...
# =============================================================================
# SECTION 3 - 32-SLOT AUDIT
# =============================================================================
print("====================================================================")
print("  [3]  32-SLOT FULL AUDIT  (mean values + zero-value detection)")
print("====================================================================")
...
try:
    if df is not None:
        ...
        for slot_key, desc in SLOT_META.items():
            ...
            mean_v = col.mean()
            if hasattr(mean_v, 'item'): mean_v = mean_v.item()
            std_v  = col.std()
            if hasattr(std_v, 'item'): std_v = std_v.item()
            min_v  = col.min()
            if hasattr(min_v, 'item'): min_v = min_v.item()
            max_v  = col.max()
            if hasattr(max_v, 'item'): max_v = max_v.item()
        ...
        if zero_slots:
            ...
            print("\n  OK  No all-zero slot columns detected. All slots look healthy.")

        if missing_slots:
            print(f"\n  ERROR  MISSING SLOT COLUMNS ({len(missing_slots)}):")
...
print(f"  data/    : {get_size('data')}")
print(f"  sigs/    : {get_size('data/signatures')}")
if os.path.exists('data/signatures_compact.parquet'):
    print(f"  compact  : {get_size('data/signatures_compact.parquet')}")
print("")
