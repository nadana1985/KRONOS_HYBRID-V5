# KRONOS V5 Mathematical & Structural Conflict Report

This document presents the authoritative quantitative audit of the KRONOS V5 hybrid engine codebase. It details eighteen mathematical inconsistencies, scale mismatches, lookahead leaks, coding errors, and dead-code blocks found across the structural, neural, validation, and data engines.
...
| 4 | Lookahead leak via `quantile(0.99)` in `slot_10` | [structural_engine.py](file:///g:/KRONOS%20HYBRID/structural_engine.py#L583) | **Critical** | Causal |
...
*   **Severity**: **Critical**
*   **Formula**:
    $$S_{\text{veto}}(t) = \text{clip}\left(\sum_{i=0}^{14} w_i \cdot S_i(t), 0.0, 1.0\right)$$
...
*   `slot_0` (`bid_ask_absorb`) outputs in $[-1.0, 1.0]$.
*   `slot_3` (`log_var_ratio`) outputs unbounded positive or negative values depending on volatility compression/expansion.
*   `slot_4` (`fractal_hurst`) outputs positive values for mean reversion ($H < 0.5$) and negative values for trend persistence ($H > 0.5$).
...
### Solution
Take the absolute values of all signed indicators, or map them onto a strict $[0.0, 1.0]$ range representing deviation magnitude before performing the weighted summation.
...
### Detailed Conflict
The weights assigned to slots $0$ to $14$ sum to $1.0$ (enforced at runtime). However, the slot outputs are completely un-aligned in magnitude and range:
*   `slot_2` (`spectral_entropy`) returns raw Shannon entropy. For a window of 72 bars, its maximum value is $\log_e(37) \approx 3.61$.
*   `slot_13` (`shannon_entropy`) returns raw Shannon entropy over 10 bins, yielding values up to $\log_e(10) \approx 2.30$.
*   `slot_6` (`ema_ribbon_deviation`), `slot_7` (`volume_price_divergence`), and `slot_12` (`microprice_deviation`) return raw z-scores which frequently exceed $\pm 3.0$.
*   Other slots (such as `slot_1` and `slot_9`) are strictly bounded in $[0.0, 1.0]$.
...
### Solution
Map each slot output onto a uniform $[0.0, 1.0]$ range (e.g. using a rolling CDF or percentile transformation) prior to weighted aggregation.
...
1.  **Path Dependency**: If a massive wick occurs early in the backtest or mining window, `norm` is locked at a high value. All subsequent wick ratios are compressed to near-zero, starving the engine of wick signals.
2.  **Scale Variance**: The representation of a bar at index $t$ changes depending on whether it is evaluated inside a single monthly shard or as part of a multi-year run, breaking the temporal consistency of the signatures database.
...
*   **Implementation**:
    ```python
    score = (wick_ratio * exhaustion_flag).clip(upper=wick_ratio.quantile(0.99))
    ```

### Detailed Conflict
The function calls `wick_ratio.quantile(0.99)` on the raw series. In python, this calculates the 99th percentile across the **entire Series** passed to the function (which, during a sharded run, corresponds to the full monthly shard). This leaks future price distribution metrics into the calculation of the current bar $t$, violating the causal boundary constraint (`max_lookahead_bars == 0`).

### Solution
Calculate the 99th percentile dynamically over a rolling lookback window (`rolling(W).quantile(0.99)`) or use a fixed configuration parameter.
...
### Detailed Conflict
The Sharpe Ratio is calculated directly on individual signature returns (`returns = mfe - mae`). Because signatures represent sparse, irregularly spaced trade entry signals, trade-level returns do not represent periodic returns. 
Multiplying a trade-level Sharpe Ratio by the square root of a daily calendar factor (`annualisation_factor: 365`) is mathematically incorrect. It assumes a constant trade frequency of exactly 1 trade per day. If a strategy trades multiple times per day, the annualised Sharpe is deflated; if it trades sparsely, it is inflated.
...
### Detailed Conflict
The "Flaw 13 Fix" was designed to anchor the neural pooling volatility gate using a long-term reference window (`global_vol_ref_bars = 10000`). However, because the input `window_df` is pre-sliced to the transformer's maximum context length (`context_length = 2048`), `len(window_df)` can never exceed 2048. 
...
### Solution
Pass the full price history series directly to the volatility computation helper instead of slicing the dataframe to the transformer's `context_length` beforehand.
...
### Detailed Conflict
The dynamic threshold is computed using `np.median(recent_convictions)` over the last 1000 signatures. Because signature signals are triggered sparsely, a 1000-count buffer can span weeks of real-time trading. 
During a sudden volatility regime shift (e.g. transition from high-volatility to low-volatility), the buffer will retain high conviction scores from the previous regime for days. This keeps the threshold artificially high, starving the pipeline of signatures. Conversely, a transition from low-vol to high-vol will leave the threshold too low, letting in low-conviction noise.

### Solution
Shift from a count-based rolling buffer to a time-based rolling window (e.g., last 7 calendar days), or scale the threshold directly by a rolling volatility index.
...
### Detailed Conflict
Synthetic buy/sell volume splits are generated using the SHA-256 hash of a bar's timestamp. This generates a random, uniform distribution in $[0.0, 1.0)$.
...
### Detailed Conflict
The configuration in `params_yaml.txt` declares `mfe_projection: {samples: 100, horizon_bars: 72}`, implying a forward Monte Carlo price excursion projection. The implementation, however, stores the `veto_score` (from `slot_15`) directly. As a result, `slot_25` is a redundant copy of `slot_15`, showing a $+1.0$ correlation. During distance calculations in HDBSCAN clustering, this redundant feature biases distances towards structural anomaly indicators.
...
### Detailed Conflict
The configuration defines `neural_regime: {latent_states: 8}`, representing a discrete regime classifier with 8 states. However, the code stores `neural_conviction` (the $L_2$ norm of the embedding vector). Instead of mapping to an ordinal regime index, `slot_26` duplicates the continuous, unbounded conviction metric.

### Solution
Perform a classification mapping to return a state index in $\{0, 1, \ldots, 7\}$ (normalised to $[0.0, 1.0]$), or use posterior probabilities from the model.
...
### Detailed Conflict
The timestamp hash is derived from the first 8 hex characters of SHA-256, yielding an integer up to $4{,}294{,}967{,}295$. When the pipeline casts features to `precision: "float16"`, this value exceeds the float16 maximum limit of $65{,}504$, causing it to saturate to `inf` or clamp. This destroys the uniqueness property of the hash. Furthermore, because of its large magnitude, it dominates Euclidean distances in downstream HDBSCAN clustering.

### Solution
Normalise the hash by dividing by the maximum hex integer ($16^8 - 1$), mapping it to $[0.0, 1.0)$, or exclude `slot_29` from clustering features.
...
### Detailed Conflict
The key `one_day_int: 1` is defined twice within the reproducibility constants block (lines 146 and 155). YAML parsers silently overwrite the first key with the second. While both keys currently share the value `1`, duplicate keys introduce configuration vulnerabilities if modified independently.
...
### Detailed Conflict
The conviction history buffer is initialized with 1000 zeros. Since the dynamic neural threshold is calculated as the rolling median of this buffer times a multiplier, the threshold remains `0.0` for the first ~500 signatures. As a result, every signature candidate that passes the structural veto during this warm-up period is unconditionally accepted by the neural gate, letting noisy signals leak into the database at the start of every month's shard.

### Solution
Initialize the conviction buffer by pre-warming over the initial warmup window, or establish a non-zero minimum floor for the neural gate.
...
$$\text{recovery} = \frac{\text{MFE}}{\text{MAE} + \epsilon}$$
When a signature is followed by a strong, clean upward trend, $\text{MAE}$ is close to zero. The division by $\epsilon = 10^{-12}$ yields recovery values up to $10^{10}$. This skews the median recovery metrics in the validation engine and dominates the distance metrics during clustering.

### Solution
Clip the recovery factor at a high sentinel value (e.g., 50.0), or use log-ratios.
...
### Detailed Conflict
`slot_7` compares price acceleration in log return space ($r_{\log} = \ln(P_t/P_{t-k})$) against volume changes computed as arithmetic percentage changes ($r_{\%} = (V_t - V_{t-k})/V_{t-k}$). The subtraction of these two mismatched scales is dominated entirely by volume swings, rendering the price signal ineffectual.
...
### Detailed Conflict
The rolling HMM state index is divided by `max_label` to normalize it to $[0.0, 1.0]$. This converts a categorical state identifier (e.g. regime 3 vs. regime 1) into a continuous intensity. When summed in `slot_15`, this introduces a systematic bias where high-volatility regimes (state 3 = 1.0) artificially inflate the composite veto score relative to low-volatility regimes (state 0 = 0.0).
...
### Detailed Conflict
The denominator is hardcoded as `const["two_int"] + epsilon`, resulting in a divisor of `2.000000000001`. Adding epsilon to a non-zero compile-time constant is mathematically redundant, reflecting an unnecessary application of the division-by-zero protection pattern.

### Solution
Simplify the division to use a clean integer divisor of `2`.
