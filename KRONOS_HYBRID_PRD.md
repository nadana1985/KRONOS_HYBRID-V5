# KRONOS Hybrid Sovereign Engine — Product Requirements Document

**File**: `KRONOS_HYBRID_PRD.md`  
...
Defines the product-level requirements, edge targets, and architectural pipeline for the KRONOS Hybrid Sovereign Engine. Every performance target, regime identifier, model parameter, slot group, and threshold resolves exclusively via dynamic configuration lookups. No inline numbers, multipliers, string labels, or dimensions appear in this document.

---

## Core Mission

Maximise trading edge through a strictly causal, bit-perfect reversal signature discovery engine that:

- Ingests raw OHLCV candlesticks at `cfg["feature_builder"]["interval"]` and tick-level aggregated trade microstructure.
...
```
RAW INPUTS
  OHLCV at cfg["feature_builder"]["interval"]
