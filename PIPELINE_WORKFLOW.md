...
Defines the complete end-to-end operational workflow for running KRONOS from cold start to a populated, query-ready signature archive. Every step references only engine function calls and config keys. No inline numbers, paths, or parameter values appear.
...
```
═══════════════════════════════════════════════════════════════
KRONOS COMPLETE PIPELINE WORKFLOW
═══════════════════════════════════════════════════════════════
...
STAGE 2 — DATA INITIALISATION
  ├── data_engine.fetch_or_load_ohlcv(cfg)            # orchestrator-grade warm reload
  │     → cfg["data"]["raw_ohlcv_path"]
  │     → incremental Binance extension + adaptive Retry-After handling
  └── miner path:
        candles_df, synth_trades_df = data_engine.load_shard_data(symbol, cfg)
          → warmup slice + cfg["data"]["warmup_bars"]
          → data_engine.generate_synthetic_trades(...)  (# "bvc" or hash-split)
...
STAGE 4 — WALK-FORWARD MINING LOOP
  └── miner_engine.run_miner(cfg, start_date, end_date)

        PER SHARD:
        ├── Linear O(N) Shard-level Precomputations
        │     → precomputed_structural_df (Structural sovereign slots)
        │     → precomputed_veto_composite (Sovereign veto score)
        │     → precomputed_vol_forecast (O(1) tail-slice, fixed O(N²) flaw)
        │
        ├── feature_builder_engine.build_full_dna_vector(
        ...
        │   )
        │     → [Linear Vector O(1) Slicing & DNA Assembly]
        │     → slot_28 set to 0.0 placeholder during mining
        │
        ├── miner_engine.compute_forward_metrics(dna_row, fwd_slice, cfg)
        │     → cfg["miner"]["forward_bars"]
        │     → cfg["reproducibility"]["constants"]["epsilon"]
        │
        │   [BATCH WRITE: once per shard, not per-bar]
        ├── database_engine.store_signatures_batch(shard_detected, cfg)
        │     → cfg["database"]["path"]
        │     → cfg["database"]["partition_by"]
        │
        │   [RUN METRICS JSON — written after shards that yielded detections]
        └── miner_engine-internal _write_run_metrics_artifact(run_metrics, cfg, symbol)
              → cfg["database"]["path"] / audit_runs / run_<symbol>_<ts>.json
        ...
        POST ALL SHARDS (per symbol):
        ├── miner_engine.compile_global_ontology(symbol, cfg)
        │     → HDBSCAN on full corpus in a single pass
...
STAGE 6 — VALIDATION SUITE
  ...
  └── validation_engine.compute_edge_metrics(signatures_df, cfg)
        ...
        → cfg["targets"]["perfect_gpf_sentinel"]  (returns 999.0 on zero-loss runs)

STAGE 7 — ANALYTICAL QUERY LAYER
  ├── database_engine.query_elite_signatures(con, cfg)
  ├── database_engine.query_by_regime(con, cfg)
...
STAGE 8 — POST-RUN OPTIMIZATION PROTOCOL
  ...
  ├── miner_engine.compile_global_ontology(symbol, cfg)  [if not already run above]
...
| :--- | :--- | :--- |
| Pre-flight | Validator, environment, seed | `cfg["validator"]`, `cfg["reproducibility"]` |
| Data initialisation | Source, symbols, intervals, warmup | `cfg["data"]` |
| Database init | Archive path, DuckDB path, pattern | `cfg["database"]` |
| Mining loop | Symbols, shard size, forward bars, gate | `cfg["miner"]`, `cfg["feature_builder"]` |
...
| Legacy microstructure flags | Older YAML knobs (`cfg["data"]["use_trades"]`) do not instantiate separate Binance AggTrades fetches — buy/sell rows are synthesized in-code | `cfg["data"]["use_trades"]`, `cfg["data"]["volume_classification_method"]` |
